...
// ---------- Minimal RouterOS API client (inlined) ----------
...
class NetSocket {
  ...
  _drain() {
    while (this.waiters.length) {
      ...
      if (total.length >= w.need) { this.chunks = [total.subarray(w.need)]; this.waiters.shift(); w.resolve(total.subarray(0, w.need)); }
      else break;
    }
  }
  ...
  async write(words) {
    ...
    while (true) {
      ...
      if (tag === "!re") {
        ...
        throw new Error("RouterOS: " + (sentence.find(w => w.startsWith("=message=")) || sentence.join(" | ")));
      }
    }
    return results;
  }
...
Deno.serve(async (req) => {
  try {
    ...
    const { router_id, interface_names } = body;
    if (!router_id || !Array.isArray(interface_names) || interface_names.length === 0) {
      return Response.json({ error: 'router_id and interface_names required' }, { status: 400 });
    }
    ...
    if (monitored.length === 0) return Response.json({ success: true, interfaces: [] });

    const now = new Date();
    const nowIso = now.toISOString();

    // Fetch existing records — only need monitored ones
    const existingRecords = await base44.asServiceRole.entities.VlanTraffic.filter(
      { router_id }, '-last_synced', 200
    );
    const existingMap = new Map();
    for (const rec of existingRecords) {
      if (nameSet.has(rec.vlan_id)) existingMap.set(rec.vlan_id, rec);
    }
    ...
    for (const iface of monitored) {
      ...
      if (exist && exist.last_synced) {
        ...
        if (secs > 0) {
          // bytes → bits (×8) → kilobits (÷1000, SI decimal — matches Mikrotik/Winbox)
      ...
      if (exist) {
        toUpdate.push({ id: exist.id, tx_kbps: txKbps, rx_kbps: rxKbps, tx_bytes: txBytes, rx_bytes: rxBytes, last_synced: nowIso });
      }
    ...
    return Response.json({ success: true, interfaces: results });
  } catch (error) {
