...
class NetSocket {
  ...
  readN(n) {
    ...
    const total = this._combined();
    if (total.length >= n) {
      this.chunks = [total.subarray(n)];
      return Promise.resolve(total.subarray(0, n));
    }
    return new Promise((resolve, reject) => this.waiters.push({ need: n, resolve, reject }));
  }
  async readByte() {
    const b = await this.readN(1);
    return b[0];
  }
  write(data) {
    return new Promise((resolve, reject) => this.socket.write(data, (e) => (e ? reject(e) : resolve())));
  }
  close() {
    try {
      this.socket.destroy();
...
async function readWord(sock) {
  ...
  if ((b0 & 0x80) === 0) len = b0;
    ...
    const b1 = await sock.readByte();
    len = ((b0 & 0x3f) << 8) | b1;
  } else if ((b0 & 0xe0) === 0xc0) {
    const b1 = await sock.readByte();
    const b2 = await sock.readByte();
    len = ((b0 & 0x1f) << 16) | (b1 << 8) | b2;
  } else if ((b0 & 0xf0) === 0xe0) {
    const b1 = await sock.readByte();
    const b2 = await sock.readByte();
    const b3 = await sock.readByte();
    len = ((b0 & 0x0f) << 24) | (b1 << 16) | (b2 << 8) | b3;
  } else {
    const b1 = await sock.readByte();
    const b2 = await sock.readByte();
    const b3 = await sock.readByte();
    len = (b0 & 0x0f) * 0x100000000 + (b1 << 16) + (b2 << 8) + b3;
  }
...
export class ROSClient {
  ...
  async login(user, pass) {
    ...
    if (ret) {
      ...
      data.set(chalBytes, 1 + passBytes.length);
