...
export default function AccountingIncome() {
  ...
  const loadData = async () => {
    try { const t = await base44.entities.AccountingTransaction.filter({ type: "income" }, "-created_date", 500); setTransactions(t); }
    catch (err) { console.error(err); } finally { setLoading(false); }
  };

  const save = async () => {
    try { await base44.entities.AccountingTransaction.create({ ...form, type: "income", amount: parseFloat(form.amount) }); setShowForm(false); loadData(); toast({ title: "Income recorded" }); }
