    ...
    <div className="p-4 lg:p-6 bg-slate-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { setForm({ recipient: "", recipient_name: "", message: "", type: "single", status: "queued" }); setBulkMode(false); setShowForm(true); }} className="flex items-center gap-2 text-xs text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg px-3 py-2 shadow-sm"><Send className="w-3.5 h-3.5" /> Send SMS</button>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <ColorStatCard label="Total Sent" value={messages.length} icon={MessageSquare} bg="bg-indigo-500" iconBg="bg-indigo-600" />
              ...
              <tbody>
                {filtered.map(m => (
                  <tr key={m.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell"><span className="text-[10px] text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded capitalize">{m.type}</span></td>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 mt-4">
            <div className="flex gap-2">
              <button onClick={() => { setBulkMode(false); setForm({ ...form, type: "single" }); }} className={`flex-1 text-xs py-2 rounded-lg border ${!bulkMode ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200"}`}>Single</button>
              <button onClick={() => { setBulkMode(true); setForm({ ...form, type: "bulk", recipient: "All Active Customers" }); }} className={`flex-1 text-xs py-2 rounded-lg border ${bulkMode ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200"}`}>Bulk to All Active</button>
            </div>
            {!bulkMode && (
              <div>
            ...
            <Button onClick={sendSMS} className="w-full bg-indigo-600 hover:bg-indigo-700"><Send className="w-4 h-4 mr-2" /> Send SMS</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
